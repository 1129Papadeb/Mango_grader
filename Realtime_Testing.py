...
def preprocess_image(img):
    ...
    # Handle float and quantized models correctly
    if dtype == np.float32:
        img_resized = img_resized.astype('float32') / 255.0
    else:
        scale, zero_point = input_details[0]['quantization']
        img_resized = img_resized.astype(dtype)
        if scale > 0:
            img_resized = img_resized / scale + zero_point
            img_resized = np.clip(img_resized, 0, 255).astype(dtype)
...
def capture_and_grade():
    ...
    # Apply softmax if output appears unnormalized
    if np.any(predictions < 0) or np.sum(predictions) != 1.0:
